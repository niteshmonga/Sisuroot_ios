...
- (void)viewDidLoad {
    ...
    [self.introcollection registerNib:[UINib nibWithNibName:@"PercievedCollectionViewCell" bundle:nil ]forCellWithReuseIdentifier:@"FirstCollectionViewCell"];
}
...
//    CGFloat prevPageX = self.introcollection.frame.size.width * MAX(_pagecontrol.currentPage, 0);
//    [self.introcollection setContentOffset:CGPointMake(prevPageX, 0) animated:YES];
...
-(void)btn4Action : (UIButton *) btn
{
    ...
    [localDict setValue:[NSString stringWithFormat:@"%ld",(long)btn.tag] forKey:@"selectedindex"];
