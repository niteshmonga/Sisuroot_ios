...
- (void)viewDidLoad {
    ...
    else
    {
        ...
        [_Addtherapistobj setUserInteractionEnabled:YES];
        [_Addtherapistobj setBackgroundImage:[UIImage imageNamed:@"sos_icon1 (2).png"] forState:UIControlStateNormal];
    }
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    if (textView == _textview)
    {
        ...
        else
        {
         ...
         //        [self.tableviewbj beginUpdates];
        //        [self.tableviewbj endUpdates];
    }
    return YES;
}
...
- (void)textViewFitToContent:(UITextView *)textView
{
    ...
    //    CGFloat fixedWidth1 = _viewobj.frame.size.width;
    //    CGSize newSize1 = [_viewobj sizeThatFits:CGSizeMake(fixedWidth1, MAXFLOAT)];
    //    CGRect newFrame1 = _viewobj.frame;
    //
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    [self.view addKeyboardPanningWithActionHandler:^(CGRect keyboardFrameInView, BOOL opening, BOOL closing) {
        ...
        CGRect toolBarFrame = _viewobj.frame;
        toolBarFrame.origin.y = keyboardFrameInView.origin.y - toolBarFrame.size.height;
        _viewobj.frame = toolBarFrame;
        
        CGRect tableViewFrame = _tableviewbj.frame;
        tableViewFrame.size.height = toolBarFrame.origin.y - 90;
        _tableviewbj.frame = tableViewFrame;
...
- (IBAction)SendBtnAction:(id)sender {
    ...
    //    {
    //        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Root" message:@"Character limit is less than 46" delegate:nil cancelButtonTitle:@"ok" otherButtonTitles:nil, nil];
    //        [alert show];
    //    }
    //    else
    //    {
...
-(void) processCompleted :(NSString *)methodeName :(NSDictionary *)methodeDictionary :(NSDictionary *)responseDictionary
{
    ...
    if ([[methodeDictionary valueForKey:@"name"] isEqualToString:@"chatapi2"])
    {
        ...
        if ([[responseDictionary valueForKey:@"status"] integerValue] ==1)
        {
            ...
            NSMutableArray *arr=[[NSMutableArray alloc]initWithArray:[responseDictionary valueForKey:@"data"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView== _tableviewbj)
    {
        ...
        //strarr1 =[newarr objectAtIndex:lastIndex];
        ...
        if ([[_detail1 valueForKey:@"chat_status"]integerValue]==1)
        {
            ...
            if ([[[strarr1 objectAtIndex:indexPath.row]valueForKey:@"group_sender_id"] isEqual:[NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults] valueForKey:@"id"]]] ) {
                ...
                // cell.mylbl.lineBreakMode = NSLineBreakByWordWrapping;
            ...
            {
                //if ([[[strarr1 objectAtIndex:indexPath.row] valueForKey:@"id"] isEqual:_detail] && [[[strarr1 objectAtIndex:indexPath.row] valueForKey:@"id"] isEqual:[NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults] valueForKey:@"id"]]])
                ...
                //  [[UIApplication sharedApplication] openURL:[NSURL URLWithString:cell.frndlbl.text]];
                
                //[[strarr1 objectAtIndex:indexPath.row]valueForKey:@"MsgTime"];
                
                // cell.frndlblview.backgroundColor = [UIColor clearColor];
                // cell.frndlbl.lineBreakMode = NSLineBreakByWordWrapping;
        ...
        else if ([[_detail1 valueForKey:@"chat_status"]integerValue]==0)
        {
            ...
            if ([[[strarr1 objectAtIndex:indexPath.row]valueForKey:@"m_senderid"] isEqual:[NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults] valueForKey:@"id"]]] && [[[strarr1 objectAtIndex:indexPath.row] valueForKey:@"m_receiverid"] isEqual:_detail] ) {
                ...
                if ([[[strarr1 objectAtIndex:indexPath.row] valueForKey:@"theorepist_status"]integerValue]==1) {
                    ...
                    //cell.frndlblview.backgroundColor = [UIColor clearColor];
                    //cell.frndtextview.lineBreakMode = NSLineBreakByWordWrapping;
                ...
                else
                {
                    ...
                    //cell.mylbl.text=[[strarr1 objectAtIndex:indexPath.row]valueForKey:@"m_msg"];
                    // cell.mylblview.backgroundColor = [UIColor clearColor];
                    
                    cell.mylblview.autoresizingMask = UIViewAutoresizingFlexibleHeight & UIViewAutoresizingFlexibleWidth;
                    //  cell.mytimelbl.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
                    ...
                    // [ cell.mylbl sizetofit];
                    //if (cell.frndtextview.text.length <1) {
                    cell.frndlblview.hidden=YES;
    ...
    else if (tableView==_Gtableviewobj)
    {
        ...
        else
        {
            if([[[imgArr3 objectAtIndex:indexPath.row] valueForKey:@"image_status"] integerValue]== 1)
            {
                ...
                [cell.Userimg setImageWithURL:[NSString stringWithFormat:@"https://w2.sisuroot.com/servicebus/profile_pic/%@",[[imgArr3 objectAtIndex:indexPath.row]valueForKey:@"profile_img"]] placeholderImage:nil];
            }
...
//    cell.mylbl.text=[strarr valueForKey:@"m_msg"];
//    cell.frndlbl.hidden=YES;
